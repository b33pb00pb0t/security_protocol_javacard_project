...
public abstract class BaseTerminal {
    ...
    protected byte[] verifyAndGetIdFromCert(byte[] fullResponse) throws Exception {
        ...
        // Cert_C: role(1) || ID_C(4) || modulus(64) || exponent(3) || masterSignature(64).
        if (fullResponse.length != ProtocolConstants.CARD_CERTIFICATE_LENGTH) {
            throw new SecurityException("Invalid certificate length received from card: "
                    + fullResponse.length + "; expected " + ProtocolConstants.CARD_CERTIFICATE_LENGTH);
        }
        ...
        byte[] signedData = new byte[ProtocolConstants.CERTIFICATE_BODY_LENGTH];
        ...
        if (role != ProtocolConstants.ROLE_CARD) {
    ...
    protected byte[] getCardCertificate() throws CardException {
        ResponseAPDU response = send(new CommandAPDU(0xB0, INS_GET_CERT, 0x00, 0x00,
        ...
        if (response.getData().length != ProtocolConstants.CARD_CERTIFICATE_LENGTH) {
            throw new CardException("Card certificate response length was " + response.getData().length
                    + "; expected " + ProtocolConstants.CARD_CERTIFICATE_LENGTH);
        }
        return response.getData();
    }

    protected PublicKey getCardPublicKeyFromCert(byte[] fullCertResponse) throws Exception {
        if (fullCertResponse == null || fullCertResponse.length != ProtocolConstants.CARD_CERTIFICATE_LENGTH) {
            throw new SecurityException("Card certificate must be exactly "
                    + ProtocolConstants.CARD_CERTIFICATE_LENGTH + " bytes");
        }
        ByteBuffer buffer = ByteBuffer.wrap(fullCertResponse);
        buffer.position(ProtocolConstants.CERT_MODULUS_OFFSET);
